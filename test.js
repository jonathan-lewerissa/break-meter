...
const { PeakTimer, speedFromGap, TABLE_DISTANCE_M } = require('./app.js');
...
// 9ft table, 200ms gap: 1.27m / 0.2s = 6.35 m/s ≈ 14.2 mph
const { mph } = speedFromGap(peaks[1] - peaks[0], TABLE_DISTANCE_M[9]);
assert.ok(Math.abs(mph - 14.2) < 0.1, `got ${mph}`);
